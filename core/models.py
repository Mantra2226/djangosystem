...
def __str__(self):
    return f"Work Order {self.work_order_id} - {self.quantity_produced} units of {self.material.name} by {self.employee.employee_name}"
...
def __str__(self):
    wo_display = self.work_order_id if self.work_order_id else "Unassigned"
    return f"Work Order {wo_display} - Step {self.step_number}: {self.instruction_text[:50]}..."      
